function suggestPrefixSumOrHashing(node, code, suggestions = []) {
  ...
  const slidingWindowRegex = /for\s*\(\s*int\s+i\s*=\s*0\s*;\s*i\s*<=\s*\w+\s*-\s*\w+\s*;\s*i\+\+\s*\)\s*{[^}]*int\s+sum\s*=\s*0\s*;[^}]*for\s*\(\s*int\s+j\s*=\s*i\s*;\s*j\s*<\s*i\s*\+\s*\w+\s*;\s*j\+\+\s*\)\s*{[^}]*sum\s*\+=\s*\w+\s*\[\s*j\s*\]\s*;[^}]*}/s;
  ...
  const binarySearchFindRegex = /for\s*\(\s*int\s+\w+\s*=\s*0\s*;\s*\w+\s*<\s*\w+\s*;\s*\w+\+\+\s*\)\s*{[^}]*if\s*\(\s*\w+\s*\[\s*\w+\s*\]\s*==\s*\w+\s*\)/s;
  ...
  const twoPointer = /for\s*\(\s*int\s+\w+\s*=\s*0\s*;\s*\w+\s*<\s*\w+\s*;\s*\w+\+\+\s*\)\s*{[^}]*for\s*\(\s*int\s+\w+\s*=\s*\w+\s*\+\s*1\s*;\s*\w+\s*<\s*\w+\s*;\s*\w+\+\+\s*\)\s*{[^}]*if\s*\(\s*\w+\s*\[\s*\w+\s*\]\s*\+\s*\w+\s*\[\s*\w+\s*\]\s*==\s*\w+\s*\)/s;

  if (twoPointer.test(loopText)) {
